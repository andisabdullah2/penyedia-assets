...
use App\Models\StatusPenumpang;
use App\Models\User;
use App\Models\OrdersPenumpang;
...
class StatusDriverController extends Controller
{
    ...
    {
        $user = Auth::user();
        $item = User::join('orders_penumpang', 'users.id', '=', 'orders_penumpang.driver_id')
                    ...
                    ->get();
        $item2 = User::join('orders_barang', 'users.id', '=', 'orders_barang.driver_id')
        ...
        return view('pages.status-driver', [
            'item'  => $item,
            'item2'  => $item2,
             ]);
    ...
    {
        $data = $request->all();
        $item = OrdersPenumpang::findOrFail($id);

        $item->update($data);

        return redirect()->route('tujuan-rute.index');
    }
    ...
    //     return redirect()->route('detail-mobil');
    // }
